...
### * Rf24_2
```{r}
freq_raw <- read_excel("Dataset.xlsx", sheet="alleles found in a colony") %>%
  as.data.frame(.) %>% 
  mutate(dominance_level = round(no_of_individuals_of_the_most_dominate_clone_type/no_secondary_queens_genotyped),2)
freq_n <- freq_raw %>% 
  filter(drive_level == "non_driven") %>% 
  separate(Rf24_2, sep = "/", c("Rf24_2", "Rf24_2_2")) %>% 
  separate(Rf6_1, sep = "/", c("Rf6_1", "Rf6_1_2")) %>% 
  separate(Rf21_1, sep = "/", c("Rf21_1", "Rf21_1_2")) %>% 
  separate(Rs15, sep = "/", c("Rs15", "Rs15_2")) %>% 
  separate(Rs10, sep = "/", c("Rs10", "Rs10_2")) %>% 
  separate(Rs78, sep = "/", c("Rs78", "Rs78_2")) %>% 
  separate(Rs68, sep = "/", c("Rs68", "Rs68_2")) %>% 
  separate(Rs02, sep = "/", c("Rs02", "Rs02_2"))
freq_n1 <- freq_n %>% 
  dplyr::select(colony,drive_level,Rf24_2,Rf6_1,Rf21_1,Rs15,Rs10,Rs78,Rs68,Rs02)
freq_n2 <- freq_n %>% 
  dplyr::select(colony,drive_level,Rf24_2_2,Rf6_1_2,Rf21_1_2,Rs15_2,Rs10_2,Rs78_2,Rs68_2,Rs02_2) %>% 
  rename(Rf24_2 = Rf24_2_2) %>% 
  rename(Rf6_1 = Rf6_1_2) %>% 
  rename(Rf21_1 = Rf21_1_2) %>% 
  rename(Rs15 = Rs15_2) %>% 
  rename(Rs10 = Rs10_2) %>% 
  rename(Rs78 = Rs78_2) %>% 
  rename(Rs68 = Rs68_2) %>% 
  rename(Rs02 = Rs02_2)
freq_dominance_level <- freq_raw %>% 
  dplyr::select(colony,dominance_level)
freq <- freq_raw %>% 
  filter(drive_level == "driven") %>% 
  dplyr::select(colony,drive_level,Rf24_2,Rf6_1,Rf21_1,Rs15,Rs10,Rs78,Rs68,Rs02) %>% 
  rbind(.,freq_n1) %>% 
  rbind(.,freq_n2) %>% 
  left_join(freq_dominance_level)
